...
from banco import *
from ai_utils import classificar_dificuldade, recomendar_data_entrega
from datetime import datetime

def criar_tela_atividades(frame):
    def carregar_atividades():
        for item in tree.get_children(): tree.delete(item)
        for at in listar_atividades(): tree.insert("", "end", values=at)

    def limpar_campos():
        entry_titulo.delete(0, "end"); text_descricao.delete("1.0", "end"); menu_turma.set("Selecione uma turma"); entry_data_entrega.delete(0, "end"); combo_dificuldade.set("")

    def inserir_atividade():
        titulo = entry_titulo.get().strip(); descricao = text_descricao.get("1.0", "end").strip()
        turma = menu_turma.get(); data_criacao = datetime.today().date().isoformat()
        dificuldade = combo_dificuldade.get().strip() or classificar_dificuldade(titulo, descricao)
        data_entrega = entry_data_entrega.get().strip() or recomendar_data_entrega(data_criacao, dificuldade)
        if not titulo: messagebox.showwarning("Atenção", "Título é obrigatório."); return
        cadastrar_atividade(titulo, descricao, data_criacao, data_entrega, dificuldade, turma if turma != "Selecione uma turma" else None)
        carregar_atividades(); limpar_campos(); messagebox.showinfo("Sucesso", "Atividade cadastrada!")
    ...
    def ao_selecionar(event):
        ...
        if sel:
            ...
            entry_data_entrega.delete(0, "end"); entry_data_entrega.insert(0, vals[4] or "")
            combo_dificuldade.set(vals[5] or ""); menu_turma.set(vals[6] or "Selecione uma turma")

    def editar_atividade():
        sel = tree.selection()
        if not sel: messagebox.showwarning("Aviso", "Selecione uma atividade para editar."); return
        id_ = tree.item(sel[0], "values")[0]; titulo = entry_titulo.get().strip(); descricao = text_descricao.get("1.0", "end").strip()
        dificuldade = combo_dificuldade.get().strip() or classificar_dificuldade(titulo, descricao); data_entrega = entry_data_entrega.get().strip(); turma = menu_turma.get()
        atualizar_atividade(id_, titulo, descricao, data_entrega, dificuldade, turma if turma != "Selecione uma turma" else None)
        carregar_atividades(); limpar_campos(); messagebox.showinfo("Sucesso", "Atividade atualizada.")

    def excluir_atividade_tree():
        sel = tree.selection()
        if not sel: messagebox.showwarning("Aviso", "Selecione uma atividade para excluir."); return
    ...
    for c,w in [("ID",60),("Título",260),("Descrição",360),("Criado em",120),("Entrega",120),("Dificuldade",100),("Turma",140)]: tree.heading(c, text=c); tree.column(c, width=w, anchor="center")
