...
from banco import (
    listar_atividades, atualizar_atividade, excluir_atividade,
    cadastrar_atividade, buscar_nome_turmas
)
...
def criar_tela_atividades(frame):
    """
    Cria a interface gráfica da aba 'Atividades'. 
    Inclui formulário para cadastro/edição, tabela de listagem e integração com o banco.
    """

    # ---------------------------------------------------------
    # Função: Carregar todas as atividades na tabela
    # ---------------------------------------------------------
    def carregar_atividades():
        """Limpa e recarrega os registros no Treeview."""
        for item in tree.get_children():
            tree.delete(item)

        for at in listar_atividades():
            # Converter datas do banco (YYYY-MM-DD) para DD/MM/YYYY
            ...
            tree.insert(
                "",
                "end",
                values=(at[0], at[1], at[2], criado, entrega, at[5], at[6])
            )

    # ---------------------------------------------------------
    # Função: Limpar todos os campos do formulário
    # ---------------------------------------------------------
    def limpar_campos():
    ...
    def formatar_data_entrada(data_str):
        ...
        Retorna None se a data estiver inválida.
        """
        try:
    ...
    # ---------------------------------------------------------
    # Função: Inserir/registrar uma nova atividade
    # ---------------------------------------------------------
    def inserir_atividade():
        ...
        # Se o usuário não escolher dificuldade, IA classifica sozinha
    ...
    # ---------------------------------------------------------
    # Função: Preencher os campos ao selecionar item na tabela
    # ---------------------------------------------------------
    def ao_selecionar(event):
        """Carrega dados da tabela para edição ao selecionar uma linha."""
        sel = tree.selection()
        if sel:
            vals = tree.item(sel[0], "values")

            entry_titulo.delete(0, "end")
            entry_titulo.insert(0, vals[1])

            text_descricao.delete("1.0", "end")
            text_descricao.insert("1.0", vals[2] or "")

            # Datas já estão no formato DD/MM/AAAA
            entry_data_entrega.delete(0, "end")
            if vals[4]:
                entry_data_entrega.insert(0, vals[4])

            combo_dificuldade.set(vals[5] or "Selecionar dificuldade")

            menu_turma.set(vals[6] or "Selecione uma turma")

    # ---------------------------------------------------------
    # Função: Editar a atividade selecionada
    # ---------------------------------------------------------
    ...
    # Atualiza lista de turmas ao abrir aba
    ...
    # Botões
    # ---------------------------------------------------------
    ctk.CTkButton(frame, text="CADASTRAR", fg_color="#1f5aa6", text_color="white",
                  width=360, command=inserir_atividade).pack(pady=(0, 6))

    ctk.CTkButton(frame, text="EDITAR", fg_color="#1f5aa6", text_color="white",
                  width=360, command=editar_atividade).pack(pady=(0, 6))

    ctk.CTkButton(frame, text="EXCLUIR", fg_color="#c94a4a", text_color="white",
                  width=360, command=excluir_atividade_tree).pack(pady=(0, 6))

    ctk.CTkButton(frame, text="LIMPAR", fg_color="#7a7a7a", text_color="white",
                  width=360, command=limpar_campos).pack(pady=(0, 6))
