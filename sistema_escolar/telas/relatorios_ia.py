...
def criar_tela_relatorios_ia(frame):
    """
    Cria a interface gráfica da aba de relatórios e IA.
    Inclui geração de PDF, estatísticas, análise de texto e respostas da IA.
    """
    ...
    def gerar_relatorio_pdf():
        """
        Gera um relatório PDF contendo todas as aulas cadastradas.
        Utiliza ReportLab e salva localmente como 'relatorio_aulas.pdf'.
        """
        dados = listar_aulas()
        ...
        y = 770  # posição inicial vertical

        # Preenchendo conteúdo do PDF
        for aula in dados:
            texto = f"{aula[1]} - {aula[2]} - {aula[3]} - {aula[4]} - {aula[5]}"
            c.drawString(50, y, texto)
            y -= 18

            # Caso a página chegue ao fim, cria uma nova
            if y < 50:
                c.showPage()
                y = 770
    ...
    def carregar_estatisticas():
        """
        Atualiza a exibição de estatísticas gerais:
        - Total de aulas
    ...
    def limpar_campos_ia():
        """
        Limpa todos os campos relacionados ao sistema de IA.
        """
    ...
    def responder_ia():
        """
        Envia a pergunta do usuário à IA e exibe a resposta no campo de texto.
        """
    ...
    # Botão gerar PDF
    ctk.CTkButton(
        frame, text="📄 GERAR RELATÓRIO PDF",
        fg_color="#1f5aa6", text_color="white",
        width=360, command=gerar_relatorio_pdf
    ).pack(pady=(10, 6))

    # Botão carregar estatísticas
    ctk.CTkButton(
        frame, text="📊 ATUALIZAR ESTATÍSTICAS",
        fg_color="#1f5aa6", text_color="white",
        width=360, command=carregar_estatisticas
    ).pack(pady=(0, 10))
    ...
    ctk.CTkLabel(
        frame, text="🧠 Assistente Inteligente",
    ...
    entry_titulo = ctk.CTkEntry(
        frame,
        placeholder_text="Título da atividade...(prova, trabalho, tarefa)",
        width=360
    )
    entry_titulo.pack(pady=(0, 6))

    ctk.CTkButton(
        frame, text="Classificar Dificuldade",
        fg_color="#4a7ac9", text_color="white",
        width=360,
    ...
    ctk.CTkButton(
        frame, text="📅 Recomendar Data de Entrega",
        fg_color="#4a7ac9", text_color="white",
        width=360,
        command=lambda: messagebox.showinfo(
            "Recomendação de Data",
    ...
    entry_texto = ctk.CTkEntry(
        frame,
        placeholder_text="Texto do aluno...(gostei, fácil, difícil)",
        width=360
    )
    entry_texto.pack(pady=(0, 6))

    ctk.CTkButton(
        frame, text="Analisar Texto do Aluno",
        fg_color="#4a7ac9", text_color="white",
        width=360,
        command=lambda: messagebox.showinfo(
            "Análise de Texto",
    ...
    # ======= Perguntas Gerais à IA =======
    ctk.CTkLabel(
        frame, text="💬 Pergunte à IA sobre o sistema:"
    ).pack(pady=(10, 4))

    entry_pergunta = ctk.CTkEntry(
        frame,
        placeholder_text="Ex: Quantas aulas existem cadastradas?",
        width=360
    )
    entry_pergunta.pack(pady=(0, 6))

    text_resposta = ctk.CTkTextbox(
        frame, width=360, height=120, state="disabled"
    )
    text_resposta.pack(pady=(0, 10))

    ctk.CTkButton(
        frame, text="Perguntar à IA",
        fg_color="#4a7ac9", text_color="white",
        width=360,
        command=responder_ia
    ).pack(pady=(0, 10))
    ...
    ctk.CTkButton(
        ...
        width=360,
        command=limpar_campos_ia
    ).pack(pady=(0, 10))

    # Carrega estatísticas automaticamente ao abrir a aba
    frame.bind(
        "<Visibility>",
        lambda e: carregar_estatisticas() if e.widget == frame else None
    )
