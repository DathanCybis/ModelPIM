...
def criar_tela_alunos(frame):
    ...
    def inserir_dados():
        ...
        if not turma or not nome:
            messagebox.showwarning("Atenção", "Preencha corretamente os campos."); return
        cadastrar_alunos(nome, datanasc, turma)
        carregar_dados_alunos(); limpar_dados(); messagebox.showinfo("Sucesso", f"{nome} foi cadastrado!")

    def editar_dados():
        selecionado = tree.selection()
        if not selecionado: messagebox.showwarning("Aviso", "Selecione uma pessoa para editar."); return
        ...
        carregar_dados_alunos(); limpar_dados(); messagebox.showinfo("Sucesso", f"Dados de {nome} foram atualizados!")

    def excluir_dados():
        selecionado = tree.selection()
        if not selecionado: messagebox.showwarning("Aviso", "Selecione uma pessoa para excluir."); return
        id_, nome = tree.item(selecionado[0], "values")[0:2]
        if messagebox.askyesno("Confirmação", f"Excluir {nome}?"):
            excluir_alunos(id_); carregar_dados_alunos(); limpar_dados(); messagebox.showinfo("Removido", f"{nome} foi excluído.")

    def limpar_dados():
        entry_nome_alunos.delete(0, "end"); entry_datnasc_alunos.delete(0, "end")
        menu_turma.set("Selecione uma turma"); tree.selection_remove(tree.selection())
    ...
    def ao_selecionar(event):
        ...
        if selecionado:
            ...
            entry_nome_alunos.delete(0, "end"); entry_datnasc_alunos.delete(0, "end")
            if aluno:
                entry_nome_alunos.insert(0, aluno[0]); entry_datnasc_alunos.insert(0, aluno[1]); menu_turma.set(aluno[2])

    entry_nome_alunos = ctk.CTkEntry(frame, placeholder_text='* Nome completo...', width=360); entry_nome_alunos.pack(pady=(8,6))
    entry_datnasc_alunos = ctk.CTkEntry(frame, placeholder_text='* Data de Nascimento (DD/MM/AAAA)...', width=360); entry_datnasc_alunos.pack(pady=(0,8))
    ...
    style = ttk.Style(frame); style.theme_use("clam")
    ...
    for col,w in [("ID",60),("Nome",420),("Idade",100),("Turma",140)]: tree.heading(col, text=col); tree.column(col, width=w, anchor="center")
